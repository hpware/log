"use client";
import { XIcon, PlayIcon, PauseIcon, Volume2Icon, VolumeXIcon, MaximizeIcon, MinimizeIcon, VideoOff, LoaderIcon } from "lucide-react";
...
export default function VideoView({
  ...
  return (
    <div
      className="fixed inset-0 bg-black/50 backdrop-blur-xl z-[100] justify-center items-center flex transition-all duration-75"
      onClick={closeState}
    >
      <div
        ref={containerRef}
        className={cn(
          "bg-gray-900 flex flex-col justify-center p-3 pt-4 z-50 mx-auto rounded-lg max-w-4xl w-full relative overflow-hidden group",
          isFullscreen ? "fixed inset-0 z-[200] max-w-none rounded-none" : ""
        )}
        onClick={(e) => e.stopPropagation()}
        onMouseMove={handleMouseMove}
        onMouseLeave={() => isPlaying && setShowControls(false)}
      >
        <div className="flex flex-row justify-between px-3 pb-3 absolute top-0 left-0 right-0 z-10 bg-gradient-to-b from-gray-900/80 to-transparent">
        ...
        <div className="relative bg-black rounded-lg overflow-hidden mt-8">
          <video
            ref={videoRef}
            src={videoSrc}
            poster={poster}
          ...
          {hasError && (
            <div className="absolute inset-0 flex flex-col items-center justify-center bg-gray-900 text-gray-400">
              ...
              <p className="text-sm">Sorry, we can't play this video right now</p>
            </div>
          )}
